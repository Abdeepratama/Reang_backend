...
<script>
  ...
  function getIconByFitur(fitur) {
    ...
    if (key.includes('sekolah') || key.includes('sd') || key.includes('smp') || key.includes('sma') || key.includes('universitas') || key.includes('kampus') || key.includes('madrasah')) {
      return iconMap.sekolah;
    }
  ...
  async function cariEksternal(keyword) {
    ...
    const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(keyword + ' sekolah')}&limit=8&addressdetails=1`;
    try {
